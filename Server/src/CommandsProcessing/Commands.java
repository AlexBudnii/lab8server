   ...
   /* public static String info(){
    ...
    public static String removeByHouseAge (long uu){
        ...
        try  {
            ...
            int j = preparedStatement.executeUpdate();
            if (j==1){
                answer = "Элементы, стаж которых  " + uu+ " созданы пользователем с логином "+ Receiver.logins.getFirst() +", успешно удалены.";
            } else if (j == 0){
                answer = "Элементы, стаж которых " + uu + " не найдены.";
            }else{
                answer = "Элементы, стаж которых превышает " + uu + " созданы пользователем с логином "+ Receiver.logins.getFirst() +", успешно удалены.";
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
